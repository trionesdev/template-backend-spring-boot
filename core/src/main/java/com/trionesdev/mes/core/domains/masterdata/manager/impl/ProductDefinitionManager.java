...
public class ProductDefinitionManager {
    ...
    public List<ProductDefinition> assembleEntityBatch(List<ProductDefinitionPO> records) {
        ...
        Map<String, ProductDefinition.ProcessFlow> processFlowMap = processFlowDAO.selectListByCodes(processFlowCodes).stream().map(processFlow -> ProductDefinition.ProcessFlow.builder().code(processFlow.getCode()).name(processFlow.getName()).build()).collect(Collectors.toMap(ProductDefinition.ProcessFlow::getCode, Function.identity()));
